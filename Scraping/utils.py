...
def get_company_info(content, url):
    try:
        ...
        company_name = (
            company_name_tag.get_text(strip=True).replace('"', "")
            if company_name_tag
            else "Unknown"
        )
        ...
        if phone_info_div:
            phone_numbers = [
                a.get_text(strip=True) for a in phone_info_div.find_all("a", href=True)
            ]
        ...
        if site_links:
            site_urls = [
                link["href"] for link in site_links if link["href"].endswith(".am")
            ]

        web_links = []
        if site_links:
            web_links = [
                link["href"] for link in site_links if not link["href"].endswith(".am")
            ]
        ...
        company_data = [
            {
                ...
                "date_of_information_update": data.get(
                    "Date of information update", "N/A"
                ),
        ...
        executive_data = (
            [{"executive": executive, "url": url} for executive in lead_infos]
            if lead_infos
            else [{"executive": "N/A", "url": url}]
        )
    ...
    for section in info_sections:
        ...
        if "Products, services by" in title:
            ...
            for first_level in prod_content.find_all("div", class_="first_level_info"):
                ...
                for second_level in second_level_ul.find_all(
                    "div", class_="second_level_info"
                ):
...
def get_company_activities(content, url):
    ...
    activity_table["activity_name_id"] = activity_table["activity_name"].apply(
        generate_id
    )
