...
if __name__ == "__main__":
    try:
        ...
        if company_table is not None:
            save_to_mysql(company_table, 'company_info')
        if executive_table is not None:
            save_to_mysql(executive_table, 'executive_info')
        if contacts_table is not None:
            save_to_mysql(contacts_table, 'contacts_info')
        if company_product_table is not None:
            save_to_mysql(company_product_table, 'company_products')
        if product_table is not None:
            save_to_mysql(product_table, 'product_info')
        if company_activity_table is not None:
            save_to_mysql(company_activity_table, 'company_activities')
        if activity_table is not None:
            save_to_mysql(activity_table, 'activity_info')
